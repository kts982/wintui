...
- **`wintui config list|get|set|unset`** — the CLI control plane for the global settings the TUI's Settings tab edits: the same 15 keys from one registry, human-readable values (`install_mode silent`, `source all`, `auto_elevate off`), strict validation (a typo is an error, never silently normalized), `unset` restores the real default, and every write is a delta over `settings.json` so a running TUI keeps its unrelated keys; `--json` returns self-describing objects
...
- **`wintui cleanup scan`** — the read-only half of the Cleanup tab from the CLI: measures what every registered target would reclaim (TARGET / GROUP / ENABLED / SIZE / ITEMS / ADMIN / STATUS), `--enabled` for the set that starts checked in the TUI, `--target <id>` to pick targets; admin-only targets are reported as `needs_admin` rather than walked unelevated (a partial walk would lie), and a walk that skipped unreadable entries is marked `partial` with a lower-bound size. Never deletes and never elevates — deletion stays in the TUI where each removal is confirmed; `--json` for scripts
