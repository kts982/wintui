...
- **Self-upgrade handoff** — WinTUI can check for its own winget update before launch and close so an inline PowerShell handoff can run `winget upgrade kts982.WinTUI` after the current process exits, without dropping a script or helper executable
- **Headless CLI** — `wintui check`, `wintui list [query]` (filter installed packages by name/id, like `winget list`), `wintui show <id>`, `wintui upgrade --all`, `wintui upgrade --auto`, `wintui upgrade --id <pkg>`, `wintui upgrade --self`, `wintui notes <id>`, `wintui theme`, `wintui config` (list/get/set/unset any global setting), `wintui rules` (list/show/set/clear per-package rules), `wintui cleanup scan` (read-only cleanup status), `wintui doctor` (verdict-first readiness check), `wintui history` (a log of WinTUI-originated upgrades), and `wintui fix --portable` (pin portable packages to user scope) for scripts, Task Scheduler, or CI without launching the TUI; `--json` works on `check`, `list`, `show`, `doctor`, `notes`, `history`, `fix --portable`, `config`, `rules`, and `cleanup scan`. WinTUI is also the human audit surface for agent-driven installs: whatever an AI agent installs or updates through winget (including the WinGet MCP server upgrade tools) shows up in `wintui check`, `wintui list`, and the Packages tab under the same policy, holds, and history. Help, usage, errors, and `--version` are styled with [fang](https://github.com/charmbracelet/fang) to match your theme, with shell completions via `wintui completion <shell>` (package IDs complete from the cache on `upgrade --id` and `show`, and from history on `history`)
...
- **`wintui config list|get|set|unset`** — the CLI control plane for the global settings the TUI's Settings tab edits: the same 16 keys from one registry, human-readable values (`install_mode silent`, `source all`, `auto_elevate off`, `cleanup_min_age 3d`), strict validation (a typo is an error, never silently normalized), `unset` restores the real default, and every write is a delta over `settings.json` so a running TUI keeps its unrelated keys; `--json` returns self-describing objects
- **`wintui rules list|show|set|clear`** — per-package rules from the CLI: `rules show <id>` prints the explicit rule next to the effective value for every field (policy, hold, scope, architecture, elevation) so inheritance is visible; `rules set <id> --policy auto --scope user --ignore-version 155.0 --elevate never` changes only the fields you pass in one write (a version hold is never silently turned into a permanent one); `rules clear <id> [field…]`; case-insensitive IDs, strict validation, `--json` with an explicit `hold` union (`none` / `all` / `version`)
- **`wintui cleanup scan`** — the read-only half of the Cleanup tab from the CLI: measures what every registered target would reclaim (TARGET / GROUP / ENABLED / SIZE / ITEMS / KEPT / ADMIN / STATUS), `--enabled` for the set that starts checked in the TUI, `--target <id>` to pick targets; admin-only targets are reported as `needs_admin` rather than walked unelevated (a partial walk would lie), a walk that skipped unreadable entries is marked `partial` with a lower-bound size, and a Core Temp target whose entries are all newer than the age floor is `recent` with the KEPT column saying what was left alone — never a bare `0 B` that contradicts Explorer. Never deletes and never elevates — deletion stays in the TUI where each removal is confirmed; `--json` for scripts
...
- **Cleanup tab** — registry-driven multi-target cleanup with grouped panels (Core Temp, Caches, Developer caches, GPU shader caches, WinTUI self-update scratch), per-target sizing, age-aware scrubbing (Core Temp targets only remove entries older than the `cleanup_min_age` floor — `1d` by default, `off` / `3d` / `7d` from Settings or `wintui config`; what the floor kept is shown next to the size, never hidden behind a bare "0 B"), and an elevated helper for admin-only paths (`%WINDIR%\Temp`, system minidumps). Configurable auto-scan policy (`safe` / `all` / `off`); developer caches default to off and opt-in toggles persist across sessions. Includes an opt-in, default-unchecked "WinTUI action history" target — a privacy reset that clears the log the History tab renders
...
# Change a setting from the CLI (validated; unset restores the default)
wintui config set install_mode silent
wintui config unset source

# Per-package rules: see what applies and why, then change one field
